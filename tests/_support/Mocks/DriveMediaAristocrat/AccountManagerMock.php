...
class AccountManagerMock
{
    ...
    public function bet()
    {
        $this->mock->shouldReceive('createTransaction')
            ...
            ->andReturn(
                $this->returnOk(TransactionRequest::STATUS_PENDING, self::BET,
                    $this->params->bet_operation_id, $this->params->balance - $this->params->amount + $this->params->winLose));
        ...
        $this->mock->shouldReceive('commitTransaction')
            ...
            ->andReturn(
                $this->returnOk(TransactionRequest::STATUS_COMPLETED, self::BET, $this->params->bet_operation_id,
        $this->params->balance - $this->params->amount + $this->params->winLose));
    ...
    public function win()
    {
        $this->mock->shouldReceive('createTransaction')
            ->withArgs(
                $this->getPendingParams($this->params->object_id, $this->params->amount, self::WIN))
            ->andReturn(
                $this->returnOk(TransactionRequest::STATUS_PENDING, self::WIN,
                    $this->params->win_operation_id, $this->params->balance));

        $this->mock->shouldReceive('commitTransaction')
            ->withArgs(
                $this->getCompletedParams($this->params->object_id, self::WIN, $this->params->win_operation_id, $this->params->amount))
            ->andReturn(
                $this->returnOk(TransactionRequest::STATUS_COMPLETED, self::WIN, $this->params->win_operation_id,
        $this->params->balance - $this->params->amount + $this->params->winLose));
